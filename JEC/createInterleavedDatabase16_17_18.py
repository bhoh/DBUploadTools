...
for db in db_import:
    ...
    if re.search(r'^[A-Z][a-z]*16_', db):
        if re.search(r'BCD_V[0-9]*', db):
            iov_list.append(("1", "276811", db))
        elif re.search(r'EF_V[0-9]*', db):
            iov_list.append(("276831", "278801", db))
        elif re.search(r'G_V[0-9]*', db):
            iov_list.append(("278802", "280385",db))
        elif re.search(r'H_V[0-9]*', db):
            iov_list.append(("280919", "294644",db))
        else:
            errorFlag=True
    elif re.search(r'^[A-Z][a-z]*17_', db):
        if re.search(r'B_V[0-9]*', db):
            iov_list.append(("294645", "299329", db))
        elif re.search(r'C_V[0-9]*', db):
            iov_list.append(("299337", "302029", db))
        elif re.search(r'DE_V[0-9]*', db):
            iov_list.append(("302030", "304826", db))
        elif re.search(r'D_V[0-9]*', db):
            iov_list.append(("302030", "303434", db))
        elif re.search(r'E_V[0-9]*', db):
            iov_list.append(("303435", "304826", db))
        elif re.search(r'F_V[0-9]*', db):
            iov_list.append(("304911", "307082", db))
        else:
            errorFlag=True
    elif re.search(r'^[A-Z][a-z]*18_', db):
        if re.search(r'A_V[0-9]*', db):
            iov_list.append(("307083", "316997", db))
        elif re.search(r'B_V[0-9]*', db):
            iov_list.append(("316998", "319312", db))
        elif re.search(r'C_V[0-9]*', db):
            iov_list.append(("319313", "320393", db))
        elif re.search(r'D_V[0-9]*', db):
