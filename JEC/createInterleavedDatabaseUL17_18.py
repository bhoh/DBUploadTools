...
for db in db_import:
    ...
    errorFlag = False
    if re.search(r'^Summer19UL18_', db):
        if re.search(r'RunA_V[0-9]*', db):
            iov_list.append(("307083", "316997", db))
        elif re.search(r'RunB_V[0-9]*', db):
            iov_list.append(("316998", "319312", db))
        elif re.search(r'RunC_V[0-9]*', db):
            iov_list.append(("319313", "320393", db))
        elif re.search(r'RunD_V[0-9]*', db):
            iov_list.append(("320394", "", db))
        else:
            errorFlag=True
    elif re.search(r'^Summer19UL17_', db):
        if re.search(r'RunB_V[0-9]*', db):
            iov_list.append(("294645", "299329", db))
        elif re.search(r'RunC_V[0-9]*', db):
            iov_list.append(("299337", "302029", db))
        elif re.search(r'RunDE_V[0-9]*', db):
            iov_list.append(("302030", "304826", db))
        elif re.search(r'RunD_V[0-9]*', db):
            iov_list.append(("302030", "303434", db))
        elif re.search(r'RunE_V[0-9]*', db):
            iov_list.append(("303435", "304826", db))
        elif re.search(r'RunF_V[0-9]*', db):
